...
class ReduceServiceTest {
  ...
  void generate_collision_linkNotInTheStore() {
    ...
    Link collisionLink = LinkBuilder.original("http://notexample.com")
                                    .identifier(keyForOriginal)
                                    .build();
  ...
  void generate_collision_linkAlreadyInTheStore(){
    ...
    Link collisionLink = LinkBuilder.original("http://notexample.com")
                                    .identifier(keyForOriginal)
                                    .build();
    Link nextAfterCollision = LinkBuilder.original("http://nextafter.ru")
                                         .identifier(nextKey)
                                         .build();
    Link nextAfterNext = LinkBuilder.original("http://nextafternext.org")
                                    .identifier(anotherNextKey)
                                    .build();
    Link testingLink = LinkBuilder.original(original)
                                  .identifier(expectedKey)
                                  .build();
